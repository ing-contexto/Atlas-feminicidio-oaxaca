  ...
  return (
    <div className="p-4 mt-8 flex flex-col flex-grow h-full">
      ...
      <div className="relative flex flex-col">
        <div className={`absolute lg:hidden top-9 left-1 p-4 shadow-lg rounded-lg z-10 ${showFilter ? "bg-gray-200 text-gray-800" : "bg-blue-600 text-white"}`} onClick={() => {
          setShowFilter(!showFilter);
        }}>{showFilter ? "Ocultar filtros" : "Mostrar filtros"}</div>
        <div className={`${showFilter ? "" : "hidden"} lg:block absolute top-9 right-4 bg-white p-4 shadow-lg rounded-lg z-10 w-64 max-h-[70vh] overflow-y-auto`}>
          <p className="text-right text-xl lg:hidden" onClick={() => setShowFilter(false)}>x</p>
          <h3 className="text-lg font-semibold mb-2 lg:mt-0 -mt-4">Filtros</h3>
